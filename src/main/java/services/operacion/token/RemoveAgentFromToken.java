...
@ApplicationScoped
public class RemoveAgentFromToken extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:removeAgentFromToken")
            .log("Removing agent from token with provided parameters")
            .doTry()
                .process(exchange -> {
                    ...
                    Boolean result = tokenService.desvincularAgente(idToken, idAgente);

                    exchange.getIn().setBody("Agente desvinculado del token exitosamente: " + result);
                })
                .to("direct:success")
